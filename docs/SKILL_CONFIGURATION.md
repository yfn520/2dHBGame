...
技能节点编辑器也可以直接编辑技能名称、描述、默认动作、技能类型、伤害倍率、冷却、射程、范围半径、弹道参数以及命中/自身 Buff 参数。新技能的执行时机统一通过节点触发方式和时间轴配置；`effect_timing` 仅为旧的无节点技能保留读取兼容，不再作为新技能的配置入口。
...
范围攻击或全屏效果节点会记录本次实际命中的目标。新增“播放特效”节点并将目标范围设置为“本次命中目标”，即可在每个命中目标的受击盒中心生成一次特效：

```json
{
  "type": "play_effect",
  "scene": "res://scenes/effects/hit_fx.tscn",
  "target_scope": "last_hit_targets",
  "trigger": "immediate"
}
```

节点顺序应为范围攻击或执行技能效果在前，播放特效在后。`target_scope: "effect_origin"` 仍表示在判定框中心、Socket 或施法者位置生成单个特效。

区域落雨使用技能的 `projectile_scene` 作为弹道视觉，然后在节点参数中选择“区域落雨”：

```json
{
  "type": "spawn_projectile",
  "projectile_mode": "area_rain",
  "projectile_count": 12,
  "projectile_interval": 0.05,
  "target_search_range": 500,
  "area_width": 240,
  "area_height": 80,
  "flight_time": 0.9,
  "gravity": 900
...
区域中心取索敌范围内最近的敌人；没有敌人时使用施法者面前的位置。每发弹道在区域内随机选择落点并沿抛物线飞入。数量和间隔控制密度，区域宽高控制覆盖范围。未选择区域落雨时，节点保持单发直线弹道兼容行为。
