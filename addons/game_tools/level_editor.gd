...
func _build_ui() -> void:
	...
	right.add_child(_make_label("刷怪点列表", 14))
...
## 判断是否能放置（data 必须是 enemy_spawn 类型且当前有关卡）
